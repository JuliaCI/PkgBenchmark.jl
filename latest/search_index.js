...

{
    ...
    "category": "section",
    "text": "You can use judge to compare benchmark results of two versions of the package.judge(pkg, [ref], baseline;\n    f=(minimum, minimum),\n    usesaved=(true, true),\n    script=defaultscript(pkg),\n    require=defaultrequire(pkg),\n    resultsdir=defaultresultsdir(pkg),\n    saveresults=true,\n    promptsave=true,\n    promptoverwrite=true)You can call showall(results) to see a comparison of all the benchmarks.Arguments:pkg is the package to benchmark\nref optional, the commit to judge. If skipped, use the current state of the package repo.\nbaseline is the commit to compare ref against.Keyword arguments:f - tuple of estimator functions - one each for from_ref, to_ref respectively\nuse_saved - similar tuple of flags, if false will not use saved results\nfor description of other keyword arguments, see benchmarkpkg"
},
